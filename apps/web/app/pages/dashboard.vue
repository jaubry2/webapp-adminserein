...
// Récupération des tâches du particulier connecté
const {
  data: taches,
  isLoading,
  isError,
  ...
  enabled: computed(() => {
    return !!session.value?.data && !session.value.isPending && isParticulier.value;
  }),
});
...
const stats = computed(() => {
  ...
  const list = taches.value as Tache[];

  return {
    total: list.length,
    aFaire: list.filter((t) => t.etat === "A_FAIRE").length,
    enCours: list.filter((t) => t.etat === "EN_COURS").length,
    terminee: list.filter((t) => t.etat === "TERMINEE").length,
    annulee: list.filter((t) => t.etat === "ANNULEE").length,
  };
});
</script>
    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <header class="space-y-4">
        ...
        <p class="text-sm quaternary--text--color">
          Gérez toutes vos tâches et démarches avec vos professionnels.
        </p>
      </header>

      <!-- Statistiques -->
      <section
        v-if="!isLoading && !isError && taches && (taches as Tache[]).length > 0"
